...
TGT_VOCAB_N_SYLLABLE = np.load('/home/users/shlohod/wiki/tgt-vocab-n_syllable.npy')
LAMBDA = 10
...
def mutant_cross_entropy_sequence_loss(logits,
                                labels,
                                sequence_length,
                                label_smoothing=0.0,
                                average_in_time=False,
  ...
  syllable_count_mean = tf.tensordot(tf.nn.softmax(logits), TGT_VOCAB_N_SYLLABLE, 1)
  loss_syllable_count_mean = tf.reduce_sum(syllable_count_mean * weights)
  loss += LAMBDA * loss_syllable_count_mean 
